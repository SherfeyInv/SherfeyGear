...
function Get-OPAFile {
    ...
    $OutFile=(Join-Path -Path $ScubaToolsDirectory -ChildPath $OPAExe)
...
function Get-ExeHash {
    ...
    $OutFile=(Join-Path $TempDir $InstallUrl.SubString($InstallUrl.LastIndexOf('/')))
    ...
    Write-Debug "Downloaded expected hash of $Hash for version $ExpectedVersion of $FileName"
    Remove-Item $OutFile
...
function Confirm-OPAHash {
    ...
    $OPAExecPath = Join-Path -Path $ScubaToolsDirectory -ChildPath $Filename
    $CalculatedHash = (Get-FileHash $OPAExecPath -Algorithm SHA256).Hash

    Write-Debug "Calculated hash of $CalculatedHash on $OPAExecPath"

    if ($CalculatedHash -ne $(Get-ExeHash -name $Filename)) {
...
function Install-OPA {
    ...
    Get-OPAFile -out $OPAExe -version $ExpectedVersion -name $Filename -ScubaToolsDirectory $ScubaToolsDirectory
    $Result = Confirm-OPAHash -out $OPAExe -version $ExpectedVersion -name $Filename -ScubaToolsDirectory $ScubaToolsDirectory
    $Result[1] | Out-Host
}
...
$OPAExecPath = Join-Path -Path $ScubaToolsDirectory -ChildPath $OPAExe

if(Test-Path -Path $OPAExecPath -PathType Leaf) {

    Write-Debug "Found installed OPA, $OPAExecPath, will confirm checksum."
    $Result = Confirm-OPAHash -out $OPAExe -version $ExpectedVersion -name $Filename -ScubaToolsDirectory $ScubaToolsDirectory
    ...
    Install-OPA -out $OPAExe -version $ExpectedVersion -name $Filename -ScubaToolsDirectory $ScubaToolsDirectory
}
